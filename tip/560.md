...
* **&lt;OptDescribe-**_name_**&gt;** — takes no arguments and returns a
  ...
  For aliases, this is a single-element list containing the name of the target
  option.
...
* **&lt;OptionsRestoreCheckpoint&gt;** — This method restores a saved state
  ...
  **next**) to restore that portion of the checkpointed state.
  ...
  If _any_ options are implemented with C backing store, an override for this
  method _should_ be provided.  Note that this means that it is probably
  unwise to override this method and not **&lt;OptionsMakeCheckpoint&gt;**.
