...
duplicates), but otherwise impose no policy on what names are acceptable.
Order within these slots is not considered to be significant.
...
classes and not just as a superclass. The instances of the classes it creates
will be the actually configurable entities.
