...
## New Option
The `encoding` ensamble will be extended by a new options `-stoponerror` `-nothrow`:
...
If `-stoponerror` is used (by default in TCL 9.0), the following error reporting takes place in case of an conversion error.
