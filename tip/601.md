...
* TCL_ENCODING_STOPONERROR (don't replace invalid characters/bytes but throw an exception on errors. Default in Tcl 9.0)
* TCL_ENCODING_NO_THROW  (don't throw an exception on errors, but replace invalid characters/bytes. Default in Tcl 8.7)
* TCL_ENCODING_MODIFIED  (convert NULL bytes to \\xC0\\x80 in stead of 0x00. Only valid for "utf-8" and "cesu-8")
...
The return value of these two functions is the error-position in case of an error, or `(size_t)-1` if everything is OK.
...
Nevertheless, an implementation is tried in the branch `tip601-failindex`.
