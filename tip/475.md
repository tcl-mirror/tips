...
interface so extension code can use it without having to enter the interpreter.
...
perform any index arithmetic of their own, they do support TIP 176 indexes.
...
`TclStringCatObjv`() returns NULL on memory allocation failure, and
`Tcl_ReplaceObj`() dutifully propagates that NULL return to its caller, with
detailed error information in the interpreter result.
