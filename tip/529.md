...
Additional possibilies in the image driver interface are proposed:
...
The image read functions (FileRead, DataRead) may return a metadata dict.
Those function get an additional tcl object pointer as "metadataOut" as parameter.

This parameter may be NULL to indicate, that no metadata return is attended (put subcommand).

This parameter is initialized to an empty unshared dict object if metadata return is attended (image create command, fconfigure subcommand).
...
The function parameters in Tk_PhotoImageFormat87 are changed as follows:
	...
	int (Tk_ImageFileMatchProc87) (Tcl_Interp *interp, Tcl_Channel chan,
	...
	int (Tk_ImageStringMatchProc87) (Tcl_Interp *interp, Tcl_Obj *dataObj,
