...
If the metadata object is writable and eventually readable by the called, a pointer to the object object (Tcl_Obj **metadataPtr) is passed.
If the metadata is only readable, the object pointer (Tcl_Obj *metadata) is passed.

If a pointer to the object pointer is passed, the following rules apply to the driver:
   ...
   *   If the pointer points to a object it must be taken care, that this object may be shared. If the metadata object should be changed, a copy must be done for a shared object.

If the object pointer is passed, the following rules apply:

   *   A NULL value indicates "no metadata".
