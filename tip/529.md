...
The TIP implementation does not target to implement all possible keys of all image formats for reading and writing immediately.
The set of predefined image keys may grow over time on a use-case basis.
...
A metadata dict given on the command line will be merged with the parsed metadata dict.
