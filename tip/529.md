...
This metadata should be accessible and setable for the following aims:
   ...
   *   Optimize operations by storing intermediate image representations. Due to that, the metadata dict may be the 3rd source (feside -file and -data options) for image data.
...
Extend the image by the property metadata.

## Image format driver interface

For image format drivers, a new registration procedure is proposed which includes functions with the metadata as a parameter:

    void Tk_CreatePhotoImageFormat87(const Tk_PhotoImageFormat87 *formatPtr)

The function parameters in Tk_PhotoImageFormat87 are changed as follows:

      int (Tk_ImageFileMatchProc87) (Tcl_Channel chan, const char *fileName,
          Tcl_Obj *format, int *widthPtr, int *heightPtr, Tcl_Interp *interp,
          Tcl_Obj **metadataPtr);
      int (Tk_ImageStringMatchProc87) (Tcl_Obj *dataObj, Tcl_Obj *format,
          int *widthPtr, int *heightPtr, Tcl_Interp *interp, Tcl_Obj **metadataPtr);
      int (Tk_ImageFileReadProc87) (Tcl_Interp *interp, Tcl_Channel chan,
          const char *fileName, Tcl_Obj *format, Tk_PhotoHandle imageHandle,
          int destX, int destY, int width, int height, int srcX, int srcY,
          Tcl_Obj **metadataPtr);
      int (Tk_ImageStringReadProc87) (Tcl_Interp *interp, Tcl_Obj *dataObj,
          Tcl_Obj *format, Tk_PhotoHandle imageHandle, int destX, int destY,
          int width, int height, int srcX, int srcY, Tcl_Obj **metadataPtr);
      int (Tk_ImageFileWriteProc87) (Tcl_Interp *interp, const char *fileName,
          Tcl_Obj *format, Tk_PhotoImageBlock *blockPtr, Tcl_Obj *metadata);
      int (Tk_ImageStringWriteProc87) (Tcl_Interp *interp, Tcl_Obj *format,
          Tk_PhotoImageBlock *blockPtr, Tcl_Obj *metadata);

If the metadata object is writable and eventually readable by the called image format driver function, a pointer to the object pointer (Tcl_Obj **metadataPtr) is passed.
If the metadata is only readable by the image driver function, the object pointer (Tcl_Obj *metadata) is passed.

If a pointer to the object pointer is passed, the following rules apply to the image format driver function:

   *   The pointer may point to a NULL value which says, that there is currently no metadata.
   *   A NULL value may be written to say, that there is no metadata to output.
   *   If the pointer points to an object it must be taken care, that this object may be shared. If the metadata object should be changed, a copy must be done for a shared object.

If the object pointer is passed, the following rules apply:

   *   A NULL value indicates "no metadata present".

## Usage of metadata dict by the functions

### Tk\_ImageFileMatchProc87, Tk\_ImageStringMatchProc87

The passed metadata may be used as image data or parameters.

The metadata creation may happen here and in the following read command.
In addition, it may be used to pass information to the read command.

The metadata may be set by intermediate or final result data.
This data may be used by a following Read command and is passed through.

### Tk\_ImageFileReadProc87, Tk\_ImageStringReadProc87

The metadata returned by the match function is passed as input.

The written metadata is the resulting metadata.

It is only shown to the script level on an image creation and configure command.
The put and read command does ignore the read metadata.

### Tk\_ImageFileWriteProc87, Tk\_ImageStringWriteProc87

The metadata is only passed for reading.
The contained keys may be used to include the data in the result data.

For example, a GIF comment may be written.
