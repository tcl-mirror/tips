...
`PROJECT_OBJS`|List of object and resource files for building the extension. The object files must be prefixed with `$(TMP_DIR)\` exactly as shown above.
...
---|---|---
`generic`|`GENERICDIR`|Contains platform-neutral source files.
`win`|`WINDIR`|Contains Windows specific source files.
`compat`|`COMPATDIR`|Contains additional portability functions not present on all platforms.

The `rules.vc` file contains implicit rules to generate the object
...
In case there are additional directories containing sources, you
will have to define additional implicit rules. For example, if
the directory `extrasrc` contained the additional sources, you would
add the line
