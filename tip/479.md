# TIP 479: Dict-Based Named Parameters in Tcl
...
TIP also calls for the creation of a new command, provisionally named *@proc*. _@proc_
...
@method as a shortcut to:

		oo::define myclass method namemethod {{{args {
		   bar {default baz}
		}}}} { ... }

The workings of @proc and @method are essentially:
...
Two macros for @proc and @method are placed in the init.tcl file to
...
The @args command is available for use at any place within Tcl, not just at the top
...
does not prohibit storing additional properties. Consider:
    ...
    @proc prettyproc {
      ...
      flavor {
       ...
       set flavors  [dict get ${@spec} flavor options]
       if {$flavor eq "random"} {
...
that @procs and @method are just convenience wrappers.
