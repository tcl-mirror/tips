...
current incarnation of **proc** provides. To that end, a local variable called **@args**
will be populated with the actual arguments fed into the call. If the procedure requires
...
4. A field name that begins with a carrot \^ will assume the options of **upvar** = **$variable** and **level** = 1.
5. If the same option is given multiple times, the last value is the one that will be used. (Similar to \[dict merge\]).
...
create a variable named **$variable** containing the value provided in the call. When
combined with **upvar**, this is simply the local name of the variable mapped from the caller.
...
Set to false if **default** given or **upvar** is non-null. If true, throw an error
if the named parameter is not given.
...
If non-null, a variable name in the caller to map this named parameter to. This is equivilent to
calling **\[upvar $level $upvar $variable\]**. If a value is given for the named parameter
it is assumed to be a replacement for the **upvar** option.
...
When upvar option is used, this option sets at what level the external variable will be
mapped.
...
technique into the existing proc command by implementing an alternative to the **args**
facility. Tentative called **@args**

		% proc p {{a {}} {@args}} { return [list a $a {*}$@args] }
		% p
		a {}
		% p foo bar
		a {} foo bar
...
in cases where the final argument is **@args**.
